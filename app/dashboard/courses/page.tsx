...
export default function CoursesCatalog() {
  ...
  const visible = useMemo(() => {
    ...
    if (q) list = list.filter(c => c.Title.toLowerCase().includes(q) || c.Reward_Skill.toLowerCase().includes(q) || c.Description.toLowerCase().includes(q));

    switch (sort) {
      case 'most-rp': list.sort((a, b) => b.Reward_RP - a.Reward_RP); break;
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 flex flex-col text-white">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 w-full space-y-7">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-4">
          <div>
            <Link href="/dashboard" className="text-slate-500 hover:text-slate-300 text-sm font-medium transition-colors inline-flex items-center gap-1 mb-2">← Dashboard</Link>
            <h1 className="text-3xl font-extrabold tracking-tight">LMS Course Catalog</h1>
            <p className="text-slate-400 text-sm mt-1">Complete courses to earn RP and unlock bounty skills.</p>
          </div>

          {!loading && !error && (
            <div className="flex gap-3 flex-wrap shrink-0">
              {[{ label: 'Total', value: stats.total, color: 'text-white' }, { label: 'Completed', value: stats.completed, color: 'text-emerald-400' }, { label: 'In Progress', value: stats.inProgress, color: 'text-blue-400' }, { label: 'Available', value: stats.available, color: 'text-slate-400' }].map(s => (
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex gap-1.5 flex-wrap shrink-0">
              {SORT_OPTIONS.map(opt => (
                <button key={opt.key} onClick={() => setSort(opt.key)} className={`px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all border ${sort === opt.key ? 'bg-blue-600 border-blue-500 text-white shadow-lg shadow-blue-900/30' : 'bg-slate-800/70 border-slate-700 text-slate-400 hover:text-white hover:border-slate-500'}`}>{opt.label}</button>
              ))}
            </div>
          </div>
        )}
        ...
        {!loading && !error && (
          <div className="flex gap-1 bg-slate-900/60 border border-slate-700/60 p-1 rounded-xl w-fit">
            {FILTER_TABS.map(tab => {
              const count = tab.key === 'all' ? allCourses.length : tab.key === 'available' ? stats.available : tab.key === 'in-progress' ? stats.inProgress : stats.completed;
              return (
                <button key={tab.key} onClick={() => setFilter(tab.key)} className={`px-4 py-1.5 rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 ${filter === tab.key ? 'bg-slate-700 text-white shadow' : 'text-slate-500 hover:text-slate-300'}`}>
                  {tab.label}
                  <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-black ${filter === tab.key ? 'bg-slate-600 text-slate-200' : 'bg-slate-800 text-slate-600'}`}>{count}</span>
                </button>
              );
            })}
          </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="bg-slate-800/40 border border-slate-700/40 rounded-2xl overflow-hidden animate-pulse"><div className="h-44 bg-slate-700/40" /><div className="p-5 space-y-3"><div className="h-4 bg-slate-700/60 rounded w-3/4" /><div className="h-3 bg-slate-700/40 rounded w-full" /><div className="h-3 bg-slate-700/40 rounded w-5/6" /><div className="h-2 bg-slate-700/40 rounded-full mt-4" /></div></div>
            ))}
          </div>
        ) : error ? (
          ...
          <>
            <p className="text-xs text-slate-600 font-medium -mt-2">Showing {visible.length} of {allCourses.length} courses</p>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {visible.map(course => {
                ...
                return (
                  <Link href={`/dashboard/courses/${course.Course_ID}`} key={course.Course_ID} className={`group flex flex-col rounded-2xl overflow-hidden border transition-all duration-300 shadow-lg ${status === 'completed' ? 'bg-slate-800/40 border-emerald-500/20 hover:border-emerald-500/40 hover:shadow-emerald-900/20' : status === 'in-progress' ? 'bg-slate-800/60 border-blue-500/20 hover:border-blue-500/50 hover:shadow-blue-900/20' : 'bg-slate-800/60 border-slate-700/50 hover:border-blue-500/40 hover:shadow-blue-900/10'}`}>
                    <div className="relative h-44 w-full bg-slate-900 overflow-hidden border-b border-slate-700/40 shrink-0">
                      {course.Cover_Image ? (
                        <img
                          ...
                          onError={e => {
                            e.currentTarget.src = 'https://placehold.co/600x400/1e293b/94a3b8?text=Image+Unavailable';
                      ...
                      <div className="absolute top-3 left-3">
                        <span className={`backdrop-blur-sm text-[10px] uppercase tracking-widest px-2.5 py-1 rounded-lg font-black shadow-lg ${status === 'completed' ? 'bg-emerald-900/80 text-emerald-300 border border-emerald-500/30' : 'bg-blue-900/80 text-blue-300 border border-blue-500/30'}`}>+{course.Reward_RP} RP</span>
                      </div>

                      <div className="absolute bottom-3 left-3">
                        <span className="bg-slate-900/85 backdrop-blur-sm border border-slate-600/60 text-slate-300 text-[10px] px-2.5 py-1 rounded-lg font-bold tracking-wider uppercase shadow">{course.Reward_Skill}</span>
                      </div>
                    </div>

                    <div className="p-5 flex flex-col flex-grow">
                      <h2 className={`text-base font-bold leading-snug line-clamp-1 mb-1.5 transition-colors ${status === 'completed' ? 'text-emerald-300 group-hover:text-emerald-200' : 'text-white group-hover:text-blue-300'}`}>{course.Title}</h2>
                      <p className="text-slate-400 text-sm leading-relaxed line-clamp-2 flex-grow mb-4">{course.Description}</p>

                      <div className="mt-auto space-y-2">
                        <div className="flex justify-between items-center text-[10px] font-bold text-slate-500 uppercase tracking-wider">
                          <span>{status === 'completed' ? `${course.Total_Modules} / ${course.Total_Modules} modules` : `${course.Completed_Modules} / ${course.Total_Modules} modules`}</span>
                          <span className={pct > 0 ? (status === 'completed' ? 'text-emerald-400' : 'text-blue-400') : 'text-slate-600'}>{pct > 0 ? `${Math.round(pct)}%` : 'Not started'}</span>
                        </div>
                        <div className="w-full bg-slate-900/80 rounded-full h-2 overflow-hidden border border-slate-700/40">
                          <div className={`h-full rounded-full transition-all duration-700 ${status === 'completed' ? 'bg-emerald-500' : 'bg-gradient-to-r from-blue-600 to-blue-400'}`} style={{ width: `${pct}%` }} />
                        </div>
                        <div className={`text-center text-xs font-bold py-2 rounded-xl mt-1 transition-all ${status === 'completed' ? 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20' : status === 'in-progress' ? 'bg-blue-500/10 text-blue-400 border border-blue-500/20 group-hover:bg-blue-500/20' : 'bg-slate-900/60 text-slate-400 border border-slate-700 group-hover:border-blue-500/30 group-hover:text-blue-300'}`}>
