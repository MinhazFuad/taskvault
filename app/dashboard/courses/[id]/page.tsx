        ...
        <div className="shrink-0 flex items-center justify-between px-5 py-4 border-b border-slate-700/60 bg-slate-900">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={() => setNotesOpen(false)}
              className="w-7 h-7 flex items-center justify-center rounded-lg text-slate-500 hover:text-white hover:bg-slate-700 transition-all text-sm">✕</button>
          </div>
        </div>
              ...
              <button key={m.Module_ID} onClick={() => !locked && setActiveModule(m)} disabled={locked}
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    {!locked && m.quiz && <span className="text-[10px] text-purple-400">🧠 quiz</span>}
                  </div>
                </div>
              </button>
            );
          ...
          <div className="mt-5 bg-slate-800/40 border border-slate-700/40 rounded-xl p-4 space-y-3">
            ...
            {isCourseComplete && (
              <button onClick={() => setShowCompletion(true)}
                className="w-full text-[10px] font-bold text-emerald-400 hover:text-emerald-300 uppercase tracking-widest bg-emerald-500/10 border border-emerald-500/20 hover:border-emerald-500/40 py-2 rounded-lg transition-all">
                🏆 View Completion
              </button>
            )}
          </div>
            ...
            <div className="p-6 space-y-6">
                      ...
                      {(['A', 'B', 'C', 'D'] as const).map(letter => {
                        ...
                        return (
                          <button key={letter} disabled={quizSubmitted} onClick={() => setQuizSelected(letter)}
                            ...
                            {quizSubmitted && isCorrect && <span className="shrink-0 text-emerald-400 text-sm">✓</span>}
                            {quizSubmitted && selected && !isCorrect && <span className="shrink-0 text-red-400 text-sm">✗</span>}
                          </button>
                        );
                      })}
                    </div>
                    {!quizSubmitted ? (
                      ...
                      <div className="space-y-3">
                        <div className={`flex items-center gap-3 px-4 py-3 rounded-xl border ${isQuizCorrect ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-300' : 'bg-red-500/10 border-red-500/30 text-red-300'}`}>
                          <span className="text-xl shrink-0">{isQuizCorrect ? '🎉' : '💡'}</span>
                          <div>
              ...
              <div className="flex items-center justify-between pt-1 border-t border-slate-700/40 gap-3">
                ...
                <div className="flex-1 flex justify-center">
                  {isCourseComplete ? (
                    <button onClick={() => setShowCompletion(true)} className="bg-emerald-600 hover:bg-emerald-500 text-white font-bold px-6 py-2.5 rounded-xl text-sm transition-colors shadow-lg shadow-emerald-900/30">🏆 View Completion</button>
                  ) : isCurrentCompleted ? (
